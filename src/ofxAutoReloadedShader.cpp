...
ofxAutoReloadedShader::ofxAutoReloadedShader() { bWatchingFiles = false; }

ofxAutoReloadedShader::~ofxAutoReloadedShader() { disableWatchFiles(); }
...
bool ofxAutoReloadedShader::load(const std::filesystem::path &shaderName) {
  return load(shaderName.string() + ".vert", shaderName.string() + ".frag",
              shaderName.string() + ".geom");
}

// ---------------------------------------------------------------------------------------------------------------------------------------------------
//
bool ofxAutoReloadedShader::load(const std::filesystem::path &vertName,
                                 const std::filesystem::path &fragName,
  ...
  if (vertexShaderBuffer.size() > 0) {
    string sourceDirectoryPath =
        ofFilePath::getEnclosingDirectory(vertexShaderFilename, false);
  ...
  if (fragmentShaderBuffer.size() > 0) {
    string sourceDirectoryPath =
        ofFilePath::getEnclosingDirectory(fragmentShaderFilename, false);
  ...
  if (geometryShaderBuffer.size() > 0) {
    string sourceDirectoryPath =
        ofFilePath::getEnclosingDirectory(geometryShaderFilename, false);
    setupShaderFromSource(GL_GEOMETRY_SHADER_EXT,
                          geometryShaderBuffer.getText(), sourceDirectoryPath);
  }
#endif
...
bool ofxAutoReloadedShader::filesChanged() {
  ...
  if (vertexShaderFile.exists()) {
    std::time_t vertexShaderFileLastChangeTime =
        getLastModified(vertexShaderFile);
  ...
  if (fragmentShaderFile.exists()) {
    std::time_t fragmentShaderFileLastChangeTime =
        getLastModified(fragmentShaderFile);
  ...
  if (geometryShaderFile.exists()) {
    std::time_t geometryShaderFileLastChangeTime =
        getLastModified(geometryShaderFile);
...
std::time_t ofxAutoReloadedShader::getLastModified(ofFile &_file) {
  if (_file.exists()) {
  std:
    filesystem::file_time_type ftt =
        std::filesystem::last_write_time(_file.path());
...
template <typename TP> std::time_t ofxAutoReloadedShader::to_time_t(TP tp) {
  using namespace std::chrono;
